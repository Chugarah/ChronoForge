...
/// 3. Foundation for type-safe repository implementations
/// </summary>
...
) : ExpressionVisitor
{
    ...
    /// <returns>Modified expression with updated parameter type</returns>
    /// <remarks>
    ...
    {
        if (node.Expression != oldParameter) return base.VisitMember(node);
        var newProperty = typeof(TTo).GetProperty(node.Member.Name);
        return Expression.Property(newParameter, newProperty!);
    }
